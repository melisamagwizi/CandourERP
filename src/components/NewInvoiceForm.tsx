...
export default function NewInvoiceForm({
  ...
  return (
    <form action={action} style={{ display: "flex", flexDirection: "column", gap: 16, maxWidth: 620 }}>
      ...
      <label style={{ fontSize: 13, color: "#6b675f" }}>Customer
        <select value={accountId} onChange={(e) => setAccountId(e.target.value)} style={{ ...input, display: "block", width: "100%", marginTop: 4 }}>
          {accounts.map((a) => <option key={a.id} value={a.id}>{a.name}</option>)}
        </select>
      </label>

      <div style={{ background: "#fff", border: "1px solid #e8e6e1", borderRadius: 14, padding: "1rem 1.25rem" }}>
        <div style={{ fontSize: 13, color: "#6b675f", marginBottom: 8 }}>Items (from your price book)</div>
        {lines.map((l, i) => {
          const p = pmap.get(l.productId);
          return (
      ...
      <div style={{ background: "#f7f6f3", borderRadius: 14, padding: "11px 14px", fontSize: 14, maxWidth: 280, alignSelf: "flex-end", width: "100%" }}>
        <div style={{ display: "flex", justifyContent: "space-between", color: "#6b675f" }}><span>Subtotal</span><span>{money(subtotal)}</span></div>
        <div style={{ display: "flex", justifyContent: "space-between", color: "#6b675f" }}><span>Tax</span><span>{money(taxTotal)}</span></div>
        <div style={{ display: "flex", justifyContent: "space-between", fontWeight: 500, borderTop: "1px solid #e8e6e1", marginTop: 4, paddingTop: 6 }}><span>Total</span><span>{money(subtotal + taxTotal)}</span></div>
      </div>

      <SubmitButton disabled={valid.length === 0} style={{ alignSelf: "flex-start", padding: "10px 18px",
        borderRadius: 10, border: "none", background: "#141414", color: "#fff", fontWeight: 500, cursor: "pointer",
